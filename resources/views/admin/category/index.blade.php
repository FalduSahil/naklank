...
@section('content')
    <div class="content-wrapper">
        @include('admin.includes.bread-crumbs', ['title' => 'Categories', 'link' => route('categories.index'), 'addOrEdit' => ''])
        <div class="content">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <a href="{{ route('categories.create') }}" type="button"
                           class="btn btn-success float-right mb-4"><i class="fa fa-plus"></i> Add Category</a>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-bordered dataTable" id="categoryTable">
                                    <thead>
                                    <tr>
                                        <th><span>#</span></th>
...
@push('scripts')
    ...
    @includeWhen(session('success'), 'admin.includes.toast', ['message' => session('edit') ? 'CATEGORY_UPDATED' : 'CATEGORY_ADDED'])
    <script>
        $(function () {
            ajaxSetup();
            let categoryTable = $('#categoryTable');
            let ajaxTable = categoryTable.DataTable({
                ajax: '{{ route('getDataTable', 'categories') }}',
                processing: true,
                serverSide: true,
                scrollX: false,
                autoWidth: false,
            ...
            categoryTable.on('click', '.change-status', function () {
                ...
                let id = $(this).data('id');
                ajaxCall('{{ route('changeStatus', 'categories') }}', {
                    'status': status,
                    'id': id
            ...
            categoryTable.on('click', '.delete', function () {
                let id = $(this).data('id');
                let url = '{{ route("categories.destroy", ":slug") }}';
